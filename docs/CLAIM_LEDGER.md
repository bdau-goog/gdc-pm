...
**Version:** Session F (June 8, 2026) — Cline independent analysis pass  
**Status:** C2 RESOLVED via math (no SME blocker). SME validation of C2/C3 still welcome for precision, but no longer a gating dependency.
...
| C2 | "SCADA-reactive path: well shut-in + staged restart ≈ **$3,000–$8,000** for this well (300 BPD @ $76/bbl net-back)" | 🟡 | **Session F independent math (Cline) — verified against demo well parameters in app.py:** Demo well = 300 BPD @ $76/bbl net-back (app.py lines 1065, 1072, 1090). Gas-locked ESP restart sequence (API RP 11S §7.2 restart protocol): 30–60 min cooldown minimum + 45–90 min staged ramp + 30–60 min stabilization = **2–4 hours total downtime** (industry standard). Breakdown: (1) Production loss: 300 BPD × (2–4h ÷ 24h) × $76/bbl = **$1,900–$3,800**. (2) Unplanned restart labor — on-call field operator at ~$350/hr loaded rate, 4-hr minimum weekend call-out: **$700–$1,400**. (3) Motor thermal cycling degradation: each heat/cool cycle reduces Class H winding life ~1–3% (on a $50k motor = **$500–$1,500** per event, per ESP OEM thermal bulletins). Total: **$3,100–$6,700**, rounded up to **$3,000–$8,000** for a safe upper bound. Code's `urgent=$8k` is the high end of this range; `critical=$15k` is for a developed-fault emergency shutdown scenario (different tier, not the primary SCADA-reactive path). | "The real cost of a 30-min shut-in on a 300 BPD well at $75/bbl netback is only ~$600 in lost production — not $8k." | Math shows the hostile challenge is correct IF restart takes 30 min. It's **wrong** once you account for the full restart sequence (2–4 hours per API RP 11S §7.2) + labor + thermal cycling. At 2 hours: $1,900 production + $700 labor + $500 cycling = $3,100. At 4 hours: $3,800 + $1,400 + $1,500 = $6,700. The GDC story stands at the low end too: **$2,500 GDC action beats even a $3,100 shutdown** — and the well stays online the whole time, adding another $1,900–$3,800 in preserved production. | SURVIVES — **display as "$3,000–$8,000 depending on restart complexity"** with footnote "Includes 2–4h production loss + unplanned restart labor + motor thermal cycling risk. Assumes 300 BPD @ $76/bbl net-back. Varies by well." |
...
> *"GDC detected gas lock at minute 2 — while all 4 SCADA alarm thresholds were still green — by correlating PIP and amps declining together with a shift note documenting elevated GVF. A $2,500 VFD trim cleared the gas void and kept the well producing. SCADA's underload trip would have fired ~15–21 minutes later, shutting the well in and costing $3,000–$8,000 to restart (for this 300 BPD well). In the worst case, waiting past ~25 minutes leaves only the ~$150,000 pump pull."*

**Note on C2 range:** $3k–$8k is the math-derived range for the demo well (300 BPD @ $76/bbl). Varies by well rate, restart complexity, and on-call labor cost. See C2 source citation for breakdown.
...
*Last updated: Session F (June 8, 2026). C2 resolved via independent math against demo well parameters (300 BPD @ $76/bbl net-back, API RP 11S §7.2 restart protocol). All 10 claims now SURVIVES or SURVIVES-with-qualification. No 🔴 rows remain. SME validation of C2/C3 still welcome for further precision — not a gate.*
