...
- The **06:00 Sonic-Survey Summary** (fluid level 150 ft above intake, flat casing pressure, no free-gas indicators → Drawdown confirmed → aggressive VFD trim contraindicated; step VFD down toward minimum flush Hz and hold, verify fluid-level headroom from this survey before any further reduction or shut-in)
...
- **Drawdown → Sand Bridging:** Dropping below the critical sand-transport velocity (≈4.2 ft/s at 52 Hz for typical WTX conditions) — whether from VFD trim or a hard shut-in — allows solids to settle and bridge in the **tubing string above the pump**, blocking restart without a workover (API RP 11S4). Spontaneous mechanical failure from dry-running alone occurs in **15–45 minutes** (SPE-174536). The RT-validated action: reduce in steps, stay above the transport-velocity floor, verify submergence from the last sonic survey.
...
| **P5-B** | Shut-in is appropriate for a moderate-sand drawdown well when documents confirm low sand cut and standing-valve integrity. It is NOT the primary GDC recommendation — the graduated step-down + hold (P5-D) is primary. Shut-in is the document-qualified endpoint if step-down fails or documents confirm it is safe. | 🟡 FIELD-PRACTICE | Independent expert review (June 10, 2026) — Gemini and Claude confirm scoped; Session BS+12 RT pass 3 adjudicates: graduated framing reconciles both — shut-in available when documents confirm low cut + standing valve | "Never shut in a sandy well." | That rule governs unconsolidated/high-cut/flowback wells. For mature Permian moderate-sand (≤0.05% cut), shut-in is defensible WHEN the completion record confirms it. GDC reads the completion record to make that determination. P5-B is now scoped to document-confirmed cases only. | **SURVIVES (scoped + document-conditional)** |
...
| **P5-D** | VFD step-down toward minimum sand-transport floor + sonic-survey verification is the RT-hardened graduated response for drawdown on a moderate-sand well where documents are available. The sonic survey (already in AlloyDB from the 06:00 shot) provides the headroom confirmation that enables the step-down in 2 seconds. Shut-in is the document-qualified endpoint if step-down is not feasible or documents confirm low cut. Graduated response requires the sonic survey — GDC's document moat. | 🟡 OUR-CODE | tab_h1.html action card + DEMO_MASTER §4 RT pass (Session BS+12, 4-pass Gemini + Session BS+13 confirmation). Reconciles P5-B (shut-in scoped, not primary) and P5-C (sandy-well rule scoped correctly). | "The step-down is useless without the sonic survey — and that's truck-roll-gated." | Correct challenge. GDC reads the LAST sonic survey (06:00 shot, already in AlloyDB). It is not ordering a new one — it retrieves an existing document in 2 seconds. That is the categorical moat: GDC reads files the RTOC cannot assemble in time. | **SURVIVES** |
