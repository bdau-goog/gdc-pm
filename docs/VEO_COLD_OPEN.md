...
**Structure:** **One Veo clip = one Vids scene = one short VO line.** ⚠️ **RESET (Session BS+33):** the cold open is LOCKED only through **Scene 5** (see RESET BANNER below). Movement 3 (the GDC solution) and the hand-off are **VOID** pending redesign next session. The old "12 scenes / Beats 5A–7" plan is retired.
...
> # ⚠️⚠️ RESET BANNER — READ BEFORE TOUCHING ANYTHING BELOW (Session BS+33) ⚠️⚠️
> **Everything from Beat 5A down to the HAND-OFF is VOID and under redesign.** The cold open is LOCKED and valid **only through Scene 5** (Movements 1–2 above). Beats 5A / 5B / 6A / 6B / 7 below are **retained for reference only** — they are NOT the plan. Do not render them as written.
>
> ### Why M3 was voided (the hard lesson from BS+32/BS+33)
> **Veo renders nouns, not concepts.** Every M1–M2 beat is a single literal subject (field, wellhead, operator, scattered desk) → renders clean. Every M3 beat asked Veo to draw an *abstraction* — a "logical/data perimeter" (5B), "data fusion" (6A) — and Veo either over-literalizes it into a hazard (explosion/liquid/lasers/steam) or draws the wrong thing (a *physical* badge-door for a *logical* perimeter). This is the medium's ceiling, not a prompt-tuning gap. **Do NOT re-attempt 5B/6A/6B as literal "fusion/perimeter" renders next session — that is the trap we already disproved.**
>
> ### THE VALUE-PROP THE SOLUTION HALF MUST LAND (the spine that was missing — make this unmissable)
> Movements 1–2 filmed **the problem** (the Diagnostic Gap: the signal says *that*, not *why*; the deciding context is scattered and slow). Movement 3 must land **the operator benefit** in one affirmative spine — three linked ideas, in order:
> 1. **MORE TIME** — GDC flags the ambiguous well early, *before* the failure becomes irreversible. *(DEMO_MASTER §1: "more time before a failure becomes irreversible. More time creates more options. More options allow cheaper, lower-risk solutions.")*
> 2. **MORE CONTEXT INGESTED** — it reads and fuses the well's own scattered documents (shift notes, lab/PVT, service logs, completion records) that no sensor system reads. *(DEMO_MASTER §3 Diagnostic Gap + L3 categorical moat: "GDC reads the documents.")*
> 3. **MORE AI / LLM-DRIVEN INSIGHT** — it turns that fused context into a **cited, reviewable diagnosis + recommended action**, in seconds, inside the operator's own perimeter. *(DEMO_MASTER §3 System A retrieval + System B Gemma; HITL approval; "automatic, fleet-scale, instant, cited, sovereign.")*
> **Net operator value:** *more time + more context + more insight → the operator responds with the right action every time, instead of a reflexive, expensive shut-in.* Sovereignty (on-prem/"AI to your data") is the **enabling WHERE**, not the headline — keep it secondary to the time/context/insight benefit.
>
> ### Two honest ways to film M3 (both keep the abstraction OUT of the frame — decide next session)
> - **PATH A — cut to the live app (the canonical V4 answer).** The cold open *is* the problem; the instant GDC becomes the subject, **dissolve into the live UI** (Beat-7 style), where fusion + cited verdict + perimeter are shown *for real* on screen. The VO bridges with the time/context/insight spine. Fewest renders, zero abstraction risk, and the app shows the truth better than any B-roll. *(See V4 §2 "What GDC Is" + DEMO_VO_PERPANEL §0.)*
> - **PATH B — 1–2 literal solution beats, VO carries the meaning.** The only renderable solution *nouns* are: **(a) the GDC hardware** (5A — already rendered & loved) and **(b) the SAME operator, now resolved** — calm, confident, acting decisively (the mirror of Scene 4's hesitation; closes the human arc). VO over these two carries time→context→insight. NO server-room "fusion" shot, NO perimeter-door shot.
> **Recommendation to open with next session:** Path A (or Path B's operator-resolved beat) + then the live demo. Confirm with the user before rendering.
...
> **🛠 STEAM FIX — starve the schema, don't negate it (BS+32, supersedes the old "dark room + no-steam" prompt).** The prior 5A said "no steam, no smoke, no vapor…" and **still rendered steam pouring off the sleds** — same failure mode as "no pumpjacks." Root cause: a **dark** equipment room + glowing LEDs + "alive with activity" + warm reflections reads to Veo as a hot, hazy, atmospheric space → heat becomes visible vapor. **Fix = positive incompatible framing, NOT negation:** make the room a **"bright, spotless, climate-controlled clean-room"** with **"perfectly clear, cold, dry, still air"** and **"bright, even, cool white lighting"** — a cold dry clean-room physically cannot have steam. The long "no steam/smoke/vapor/mist/fog" tail was **removed** (naming the tokens fed the schema). Dropped the moody "dark room / cool blue key" grade for the same reason.
> **🛠 ADD MOTION (your note — "zero motion, bland"):** Veo renders a *still life* unless you script movement explicitly. Two motion layers drive the shot: (1) **camera** — a slow left-to-right **tracking glide** with a gentle **rack-focus** from near sled to far (a static hero shot of hardware always reads as a product photo); (2) **in-frame life** — LEDs **blinking** (data traffic), a **specular highlight traveling** across the metal as the camera moves, fiber cables that **catch the light**. **Do NOT use heat-shimmer / "warm exhaust air" as a motion layer** — that's exactly what produced the steam. If still too static, escalate the camera to a slow **push-in toward the blinking LED bank** and add *"LEDs pulsing in sequence, a wave of light running along the row."*
...
**🎬 Beat 5B — inside the operator's perimeter (sovereign)**  ·  *~6s*  ·  *(STANDALONE access-control shot — NOT a continuation of 5A's room)*
```
Photorealistic cinematic close shot of a secure glass entry door to a private on-premise server room. Mounted beside the door at hand height is a small illuminated access-card reader glowing soft green. Through the glass, slightly soft and out of focus behind the door, the cool glow of quiet equipment cabinets with small green and blue status lights is just visible. A hand holding an access badge moves toward the reader; the reader light blinks in acknowledgment. The mood is controlled, private, secure — a restricted, access-only facility. Cool even lighting, clean and calm, shallow depth of field, slow steady motion. No readable text, no logos, no screen content; any labels stay soft and out of focus.
```
> **🛠 5B = STANDALONE ACCESS-CONTROL SHOT, do NOT try to continue 5A's room (BS+32 — two failed renders settled this).** We tried both ways and both failed: (a) feeding a 5A still → Veo tiled MORE of the same sleds, no room revealed; (b) cold "wide pull-back" → Veo zoomed IN to a completely different data center, no continuity. Conclusion: **Veo cannot bridge 5A's macro to a wider room, in either direction.** Fix = stop trying. Make 5B a **conceptually distinct image that needs no continuity**: the **secure door + glowing badge reader** (access control). The cut 5A servers → 5B secure door reads as **intentional** (two ideas in sequence), not a broken match — nobody expects a badge-reader shot to match a server macro. Bonus: making the **door the subject** kills the camera-direction problem (Veo ignored "pull-back" and zoomed in — there's no camera move to botch when the door is the subject). The servers stay **soft behind the glass** for a whisper of 5A's room without needing it to match.
...
> **🛠 5B = COLD WIDE SHOT, do NOT use a 5A still as the ingredient (BS+32 render lesson — supersedes the old "Extend the pull-back" plan).** We tried feeding a 5A frame (macro on the sleds) and prompting a pull-back; Veo **just tiled more of the same sleds** instead of revealing a room. Root cause: a tight macro ingredient anchors a *texture* ("wall-to-wall server hardware"), not a *camera move* — it has no room to invent a glass door or a wider RTOC. The pull-back-reveal was Extend's specialty (a real motion), and Extend is broken. **Fix: generate 5B COLD (no ingredient) as its own wide establishing shot.** Carry continuity through *matching words* — "a single short stack of slim silver-white compute sleds" + same cool clean grade — NOT through the 5A still. Minor grade drift across the macro→room cut is forgiven (per the win-condition rule); the VO + hard location change cover it. **Cap the scale:** say "small / compact private server room, not a vast data hall" so Veo doesn't build a hyperscale hall. The glass door + badge reader carries the security idea that IS 5B.
...
### Flow mechanics — settled BS+32 (don't re-litigate)
- **Clip length is ~8–10s, NOT hard-locked at 8s.** Veo 3.1 in Flow treats the "8s" selector as a target and may render up to ~10s. This is fine — our VO lines are written to ~7–8s, so the extra 1–2s lets the visual breathe (trim head/tail in Vids). The older 8s "hard cap" was the Vids-embedded Veo; Flow's Veo 3.1 has loosened it. At 10s, glance at the final ~2s of operator (Mark) and hardware shots for coherence drift; trim the tail in Vids if a face/server morphs.
- **Ingredients use Nano Banana Pro (the image model), NOT Nano Banana 2.** Nano Banana = Google's still-image model; in Flow it generates/edits the reference Ingredients (well-field, sled stack, server room, Mark portrait) that Veo then animates. Use **Pro** — better text suppression + detail/negative adherence, which matters for fighting garbled hardware labels and pumpjack bias. Credits aren't a constraint.
- **Veo's video-output count is set on the VIDEO generation bar, NOT in the Agent-settings "Image generation default" panel.** The x4 in the image-default panel only makes 4 *image* variations (Ingredients); it does nothing for video. Veo defaults to 1 clip per generation. To get the x4 "best-of-4 against the steam/spark/pumpjack lottery," set outputs on the Veo bar itself, or simply re-run the same prompt 3–4 times.
- **MODEL CHOICE — settled BS+32, do NOT re-litigate. Standard = Omni Flash for ALL 12 beats.** Two hard facts forced this: (1) **Extend is BROKEN in Flow right now** — on every model tried (incl. Veo 3.1 Lite) it pretends to generate but returns **blank video**. So Extend is OFF THE TABLE regardless of model; the only working continuity mechanism is **reference images / still-frames**. (2) **Veo 3.1 Quality CANNOT use reference images** (Flow warns "this model doesn't support using reference images…"; attaching an ingredient on Quality SILENTLY renders text-only — this is why the first Beat 1 ignored the well-field ingredient). With Extend dead, the only thing that matters is reference support → **Omni Flash** (references ✅, 10s clips) is the pick. Veo 3.1 Lite also supports references but caps at 8s and gives no Extend benefit (broken anyway), so Omni Flash wins on clip length. References are non-negotiable: M1 (Beat 2 needs a Beat 1 frame) and M2 (Beats 3/4A/4B/4C need the **Mark** Character) collapse without them.
- **CONTINUITY = still-frame ingredients (the ONLY method now that Extend is broken).** This was always the doc's global rule. Build order: each beat that must match a prior one takes a **still frame from the prior clip as its ingredient**, and the prompt describes only the *change*. M3: 5A (macro on sleds, cold) → 5A last-frame as ingredient → 5B ("pull back to reveal the room"); 5B/6A/6B also share the **server-aisle ingredient**; 6A last-frame → 6B. One model = one uniform grade across all 12 beats. Win condition is **clips that cut together under per-scene VO**, NOT frame-perfect continuity — the per-scene VO structure forgives minor grade drift.
