...
**Structure:** **One Veo clip = one Vids scene = one short VO line.** STATUS (Session BS+36): the cold open is a **9-scene** structure (Scenes 1-9), all scenes designed and approved. **Scenes 1-7 are DONE** (Scenes 1-6 rendered; Scene 7 = POV cut assembled in Vids as two sub-scenes). **Scenes 8-9 remain**: operator-resolved fusion + HITL (Scene 8), and the dissolve hand-off (Scene 9). Movement 3 redesign is **APPROVED** (not void). The old "12 scenes / Beats 4A-4C, 5A-5B, 6A-6B" plan is **retired** — the problem half collapsed old Beat 3+4A into one Scene 3, and the solution half is now three literal scenes (Beats 5/6/7) plus the hand-off.
...
> **PRODUCTION TOOL — Scenes 7-9 (Session BS+35/BS+36).** Remaining solution-half beats are made in **Vids/Veo using EXTEND.** Scene 7 done (POV cut: 7A Veo render + 7B real screenshot). Scene 8 = Extend from 7A Mark render (nod + authorize). Scene 9 = standalone Vids build from real screen-recording. Scene 6's sled clip is already rendered and reused as-is. The Flow process notes below are retained as history for the already-rendered Scenes 1-6.

### SCENE BEAT MAP (the canonical 9-scene order)
| Scene | Beat | Subject | Render status |
|---|---|---|---|
| 1 | Beat 1 | Aerial field of blue christmas-tree wellheads | DONE rendered |
| 2 | Beat 2 | Push-in: one wellhead + control/VFD skid | DONE rendered |
| 3 | Beat 3 (merges old 3 + 4A) | Night alarm + "often easily diagnosed" concession | DONE rendered |
| 4 | Beat 4B | The ambiguous alarm (operator hesitates) | DONE rendered |
| 5 | Beat 4C | Scattered context (operator hunts across monitors + tablet) | DONE rendered |
...
## DEFEATING VEO'S PUMPJACK BIAS (read first — negative prompts DON'T work)
...
> **CONTINUITY IS DONE WITH A REFERENCE IMAGE OR EXTEND, NOT WITH WORDS.** Veo has **no memory between cold generations** — phrases like "the same desk/operator as Beat 3" in these prompts are only loose hints and will NOT reliably reproduce the prior shot. To make beats actually match, either (a) **feed a still frame from the prior clip as a reference/ingredient image**, or (b) **use Vids/Veo EXTEND** to continue the prior clip (BS+35/BS+36 method for Scenes 7 to 8). Then use the prompt to describe only the change (new angle, new action). Treat all "same as Beat N" prose below as intent for the human, not as something Veo enforces.

> **VEO 3.1 DIALOGUE TRAP (BS+36 — permanent guardrail):** Veo 3.1 generates synthetic spoken dialogue and audio when a prompt describes a character doing something decisive (head turns, keystrokes, nods, active scanning). It invents lines like "I've got a lock on the secondary feed." **Prevention:** always include the no-dialogue suppression clause in every character render prompt: *"No spoken dialogue, no voiceover, no narration, no talking, no radio chatter, no comms audio. The operator does NOT speak and his lips do not move. Ambient room tone only — no discernible words. Silent cinematic shot."* **Post:** MUTE every Veo clip's audio track in Vids before adding your recorded VO. Treat ALL Veo clips as picture-only. Applies to all operator scenes (3, 4, 5, 7A, 8).
...
```
> **VO — Beat 1 (Scene 1):** *"Upstream oil and gas runs on engineering discipline — lifting every barrel as efficiently and safely as possible."* · *(~17 words / ~8s)*
...
```
> **VO — Beat 2 (Scene 2):** *"Electric submersible pumps - ESPs - do that lifting. Maintaining them well keeps costs down and optimizes production."* · *(FINALIZED wording, Session BS+33 — verbatim)*
> **Note:** this is the **same scene as Beat 1, just closer** — one wellhead sharp in front, the field soft behind. The pump itself is downhole (invisible); the surface story is the blue valve tree + the control/VFD skid beside it. Same hardware and grade as Beat 1 keep the wide-to-tight cut continuous; the tight framing leaves no room for a jack.
...
> **GROUNDING — "pump connected to nothing" fix (BS+32 render):** an earlier render produced a **floating, disconnected pump-like object**. Two causes: (1) the word **"pump"** summons a surface skid-pump schema — **keep "pump" OUT of the visual prompt** (the ESP is downhole/invisible; "pump" lives only in the VO); (2) nothing forced the hardware to be plumbed in. **Fix = ground everything:** tree **"bolted to a low concrete base,"** an explicit **flowline to gathering-pipe tie-in**, skid **"linked by conduit,"** and a hard clause: **"Every piece of equipment is solidly connected — nothing floating or detached."**
...
```
> **VO — Beat 4A:** MERGED into Scene 3 (BS+33). The "easily diagnosed" concession now lives in Scene 3's second sentence. Do NOT render 4A separately.
...
> **Flow (3-beat concede to pivot to friction):** Scene 3 *"often easily diagnosed"* → Scene 4 (4B) *"But some are ambiguous — the signal alone can't tell you the cause"* → Scene 5 (4C) *"The context that can sits scattered..."*. The word **"can"** links Scene 4-5 (signal *can't* / context that *can* is scattered). Record each line on its own scene.
...
**Beat 4C (Scene 5) — the scattered context (FRAGMENTATION is the subject)** · *~8s+ (use EXTEND if VO runs long)* · *(same console as Scenes 3/4; slower, heavier)* · *RENDERED (BS+35 over-the-shoulder take)*
```
Photorealistic cinematic over-the-shoulder shot of the same control-room operator from the previous scenes, seated at the same dark RTOC console at night, in three-quarter profile, looking off-camera at his monitors — he does NOT look at the camera. Same navy work shirt, same cool blue screen glow with a warm amber desk-lamp accent. He is searching for context: his eyes move between several separate monitors that each show a different, incompatible software interface, and he picks up a rugged tablet from the desk and studies it, then glances back to a monitor, cross-referencing information that lives in disconnected systems. Every screen — the monitors and the tablet — stays soft, blurred, and out of focus, reading only as colored software layouts, header bars and tables, with no legible text or numbers. He has exactly two hands, anatomically normal; one holds the tablet while the other rests near the keyboard. The desk surface is clean — only the keyboard, mouse, and the tablet — no paper, no printed sheets, no documents, no notebooks. Slow, deliberate, slightly tense pacing, conveying the effort of manually assembling scattered context under a live-event clock. Shallow depth of field, 35mm film look, cool blue and amber night-shift grade matching the previous scenes. No legible text, no readable numbers, no logos, no recognizable UI chrome. No spoken dialogue, no voiceover, no narration, no talking. The operator does NOT speak and his lips do not move. Ambient room tone only. Silent cinematic shot.
```
> **VO — Beat 4C (Scene 5):** *"The context that can help is scattered across distributed systems — slow to assemble when the decision can't wait."* · *(FINALIZED wording, Session BS+33 — verbatim)*
> **FRAMING (BS+35 redirect — over-the-shoulder, NOT overhead):** the approved render keeps Mark at his console (contiguous with Scenes 3/4) and tells fragmentation through **where he looks** — his eyes jumping between several incompatible monitors plus a rugged tablet he picks up to cross-reference. This replaced the earlier overhead top-down desk shot, which (a) reintroduced **paper + garbled fake text** and (b) grew a **third hand** (Veo extra-limb glitch when two hands are told to work three spread-out devices). The over-the-shoulder framing fixes all three: no overhead cut to break continuity, context lives on the screens not the desk, and a normally-seated operator can't sprout an extra hand.
> **VO intent (the real friction):** the deciding context **exists** — but it's **fragmented across disconnected digital systems** (CMMS, lab portal, handover log, file shares) and **slow to find, combine, and decide from** under a live-event clock. Convey *fragmentation + retrieval/assembly friction*, not *distance* or *absence*. The fragmentation is shown through the operator hunting across several incompatible monitors plus a rugged tablet (NOT desk clutter). This is exactly the H1/H2 demo subset (look-alike telemetry: gas-interference-vs-drawdown, paraffin-mimicking-bearing-wear) and what GDC collapses into a cited diagnosis in seconds.
...
> **Why digital-only fragmentation (TRUTH gate):** a modern RTOC engineer's sources are entirely digital (PDFs, CMMS like Maximo/SAP, a digital handover log, shared files). Having paper or printed sheets on an RTOC desk is unrealistic. The fragmentation is purely digital: the data lives in **different, incompatible systems** that don't talk to each other. The operator hunting between several incompatible monitors and a rugged tablet makes "disconnected systems" literal.
> **Veo garble dodge:** every screen risks fake text. Keep **all on-screen applications slightly soft / out of crisp focus** so they read as software layouts (headers, tables, loading spinners) without legible words. Reinforce *"no readable text, no recognizable UI chrome."* The earlier overhead-desk attempts rendered crisp garbled report text ("Repirbort / Lag-Re-Reart") — sharp fake words are worse than no text; keep every screen soft.
> **Continuity (do this, not the prose):** feed a still frame from Scene 3/4 (or the approved Scene 5 over-the-shoulder take) as a **reference ingredient** so the operator/desk/grade match — prompt prose like "same console" won't reproduce it on its own. Use the prompt only to specify the *change* (he picks up the tablet, eyes hunting between monitors). Keep the color/lighting identical so the cut reads as the same room and the same Mark.
> **INTEGRITY GUARDS (BS+35):** "exactly two hands, anatomically normal" (blocks the third-hand glitch); every screen "soft, blurred, no legible text" (blocks garble); hardened "no paper / no printed sheets / no documents" (honors the BS+34 de-paperization decision — RTOC sources are digital). Displayed scene must match the digital-fragmentation message.
...
```
Photorealistic cinematic macro shot inside a bright, spotless, climate-controlled enterprise data clean-room with perfectly clear, cold, dry, still air. The camera performs a slow, smooth tracking glide from left to right along the front edges of a small stack of three identical slim silver-white enterprise compute sleds mounted one above another, the focus racking gently from the near sled to the far one. Across the clean front faces, dozens of tiny status LEDs blink in green and amber, while a single steady blue power LED glows on each unit. A crisp specular highlight travels slowly across the brushed-metal surfaces as the camera moves. A few blue and orange fiber patch cables catch the moving light. The dense ports stay soft and out of focus so they read as fine texture, never text. Bright, even, cool white lighting; clean dry air; immaculate surfaces. Only the three stacked sleds are in frame — no surrounding rack frame, no power strips, no outlets. Crisp, controlled, enterprise-grade, on-premise.
```

> **VO — Beat 5 (Scene 6):** *"This is where Google Distributed Cloud comes in — it brings Google's AI to your data, instead of your data to the cloud."* · *(RENDERED wording — verbatim to the delivered clip, BS+35)*
...
**Beat 6 (Scene 7) — Scoring the Combined Pattern (Pre-Threshold Faster Detection)** · *~9-10s total (7A ~3s + 7B ~6-7s)* · *DONE — two sub-scenes assembled in Vids*

```
POV CUT — two sub-scenes assembled in Vids (BS+36 final):

7A (~3s) — Veo render DONE (hard down-left, audio suppressed):
Photorealistic cinematic medium shot of the RTOC operator from the reference portrait — dark short hair, dark navy work shirt — seated at a control-room desk at night, framed on the right side of the shot in three-quarter profile. The single dominant action: he sharply drops his head down and to his left, turning decisively to study a low workstation monitor sitting on the desk at the far lower-left of frame — chin dropping toward his chest, eyes cast down and to the left at that near screen. It is a strong, deliberate, unmistakable downward-left head movement, not a subtle glance. He holds that hard down-left gaze, focused and intent, through the end of the clip. He stays in three-quarter profile and NEVER turns his face toward the camera; his chin stays low and his eyes stay down on the desk monitor. His face and near shoulder are lit by a shifting multi-channel screen-light palette — cool steel-blue base with teal, soft amber, and warm gold traces that pulse as live data flows. A small warm amber desk-lamp glows in the lower-left foreground beside the low monitor, casting a soft orange accent across his lowered jaw. The distant wall of control-room monitors behind him stays soft blue and teal bokeh. Shallow depth of field, 35mm film look, cool blue dominant with teal and amber tints. No legible screen content anywhere — no text, no numbers, no logos, no UI chrome. No spoken dialogue, no voiceover, no narration, no talking, no radio chatter, no comms audio. The operator does NOT speak and his lips do not move. Ambient room tone only — no discernible words. Silent cinematic shot.

7B (~6-7s) — Real GDC Advisor screenshot, NO VEO — DONE:
  - Fade in from 7A's down-left look.
  - START: chart stack, BOTH markers in frame — amber "GDC" (~9 min) + red "Threshold SCADA" (~18 min). Hold ~1.5s on the gap.
  - DRIFT: slow push-in + pan up-right along the declining PIP/Current curves.
  - SETTLE: land on the Decision Console panel showing "WELL A-3 — BASELINE MONITORING · hs = 0.9029 · XGBoost routing score" and the "RETRIEVED CONTEXT — ALLOYDB PGVECTOR (< 2S)" header (loading state, no doc cards, no recommendation yet).
  - This is the DETECTION-MOMENT screenshot: GDC has fired, docs are being fetched, no recommendation visible.
  - MUTE the clip audio track in Vids (treat all clips as picture-only).

Assemble: 7A fade to 7B, then cut to Scene 8.
```
...
> **AUDIO — VEO 3.1 DIALOGUE TRAP (BS+36):** Veo 3.1 generates synthetic dialogue when a prompt describes decisive action (head turns, keystrokes, nods). It invents lines like "I've got a lock on the secondary feed." Prevention: always include the no-dialogue suppression clause (above, in every character prompt). Post: MUTE every Veo clip's audio track in Vids. Treat all Veo clips as picture-only. **Applies to Scene 8 Extend as well.**
> **Continuity / PRODUCTION:** 7A render is the **Extend base for Scene 8** — Extend from 7A (Mark's hard down-left focus) into Scene 8 (expression resolves to calm certainty then nod then authorize keystroke). Scene 9 uses the same screenshot/recording as 7B (slow push-in + brighten dissolve into the live demo).

---

**Beat 7 (Scene 8) — Fusing the Unstructured Context (The Context Problem Resolved + HITL)** · *~7s* · *TO RENDER (fresh cold render — NOT Extend; lamp-out is the dominant event)*

```
FRESH COLD RENDER — do NOT use Extend (Extend kept the lamp on). Use a still frame from the 7A clip as a reference ingredient for face/wardrobe continuity, plus the Mark portrait.

Photorealistic cinematic medium shot of the same RTOC operator from the reference still frame — same dark navy work shirt, same desk, same cool blue and amber RTOC night-shift grade. He is in three-quarter side profile, looking off-camera at his workstation monitors — he NEVER turns toward the camera. The dominant visual event of this clip unfolds in four beats: (1) his expression holds quiet, satisfied certainty — the relaxed composure of a resolved decision; (2) with a calm, deliberate gesture, only one hand reaches forward and taps a single key on the keyboard to authorize; (3) immediately after the keystroke, the warm amber desk-lamp on the desk — which has been glowing this entire scene — dims, fades, and goes completely dark, leaving the desk lit only by cool blue monitor light; (4) Mark settles back slightly, composed and at ease in the now-calm blue light, the tension entirely gone. The lamp-out is unmistakable and clearly visible — a definitive transition from warm amber warning-state to calm cool-blue resolved-state. He has exactly two hands, anatomically normal with five fingers each. Distant monitors stay completely out of focus as soft blue bokeh. Shallow depth of field, 35mm film look. No readable text on any screen, no logos. No spoken dialogue, no voiceover, no narration, no talking, no radio chatter, no comms audio. The operator does NOT speak and his lips do not move. Ambient room tone only — no discernible words. Silent cinematic shot.
```

> **VO — Beat 7 (Scene 8) — FINAL (BS+36):** *"GDC then fuses your live documents with that data — diagnoses the fault, prepares the fix, and awaits your authorization."* · *(17 words / ~7s)*
...
Convey "resolution" through Mark's relaxed body language, the decisive nod, and the deliberate keyboard authorize action, even as the amber alert light remains glowing on the desk (it only settles once the action is executed in the live demo).
> **Continuity / PRODUCTION (BS+36 final):** **FRESH COLD RENDER — do NOT Extend.** Extend kept the lamp on (wrong). Use a still frame from 7A as an ingredient for face/wardrobe continuity. The lamp MUST go out — it is the dominant visual event. MUTE the Veo clip audio in Vids. No-dialogue suppression clause required.
...
## PRODUCTION PROCESS — TOOLING (BS+35/BS+36 update at top; Flow history retained below)

> **CURRENT METHOD (Session BS+35/BS+36) for the remaining solution-half beats (Scenes 8-9):** Scene 8 = Extend from 7A in Vids/Veo; Scene 9 = Vids build from real screen-recording. Scenes 1-7 are done. This supersedes the Flow/Omni-Flash approach below **for the remaining beats** — that approach is retained only as the record of how Scenes 1-6 were produced.

**Why Flow was used for Scenes 1-6 (history):** every failure in the original Vids-embedded Veo workflow traced to **no cross-shot context** — Veo regenerated cold each time (re-inventing servers), and the only way to steer it was retyping the whole prompt. That produced the explosion to liquid to laser-beam churn on the abstract "fusion" beat. Flow's Characters + Ingredients gave the cross-shot consistency the per-beat prompts assume, and was the right tool for the problem-half operator beats and the sled macro.
...
- **CONTINUITY methods:** (a) **Extend** (BS+35, Scenes 7 to 8 — preferred where it works); (b) **still-frame reference image** (the fallback used for Scenes 1-6 — each beat that must match a prior one takes a still frame from the prior clip and the prompt describes only the change). Win condition is **clips that cut together under per-scene VO**, NOT frame-perfect continuity — the per-scene VO structure forgives minor grade drift.
...
**Guardrails (unchanged, any tool):** no readable screens/UI (garble); no Dell branding + never feed the Dell photo as an ingredient; hard-negate steam/sparks/fire/liquid/beams; operator three-quarter side profile, never to camera; schema-starve outdoor beats (blue valve-tree, never "oil field/pumpjack"); **no spoken dialogue / operator lips do not move / mute all Veo audio in Vids (BS+36 permanent addition).**
...
1. **One scene per beat.** You'll have **9 intro scenes** (Scenes 1-9 per the SCENE BEAT MAP at the top); drop each beat's Veo clip on its scene. Scene 7 = two sub-scenes (7A Mark clip + 7B screenshot) with the VO spanning both.
...
6. **Prefer to narrate elsewhere?** Record each line as its own audio file (phone/QuickTime) and **Insert then Audio then Upload** onto the matching scene. Same one-line-per-scene principle.

> **Tips:** leave ~0.3s of silence at the head of each take so cuts don't clip the first word. If a clip is shorter than its line, extend the scene or slow the clip slightly; if longer, let the visual breathe before cutting. **For Scene 7 (two sub-scenes):** record the full 7A+7B VO as one continuous take and drop it spanning both; the pause falls on the 7A-to-7B fade.
...
- **Continuity:** all 9 scenes play **back-to-back**. Aim for similar grade/color across clips so the cut feels continuous even though each carries its own VO line. **Veo has no cross-generation memory** — to actually match a prior shot (operator, desk, wellhead, server), use **Extend** (Scenes 7 to 8) or **feed a still frame of that clip as a reference/ingredient image**, and let the prompt describe only the change. "Same as Beat N" prose is a hint for you, not an instruction Veo can follow.
- **Minimum viable intro** if short on time: Scenes **1, 3, 5, 6, 8** still tell the whole story (context to problem to solution to demo) in ~40s. (Scene 2 ESP detail and Scene 4 hesitation are the first to trim; Scene 5 carries the core "scattered context" value point and Scene 8 carries the resolution.)
