...
Date: 2026-06-16 / git head: 72a332a / branch: feature-trio-clean
...
- **FEAT (72a332a):** H1 legibility pass — 0.52-0.62rem font floor throughout Decision Console.
  Zone widths corrected 58/30/15 → 56/28/14 (wellbore SVG no longer clipped). X-MAS → HEAD in SVG.
  App-level +/- zoom: keydown handler adjusts body font-size (all rem sizes scale); persists localStorage;
  toast shows zoom %. `0` resets to 100%. Range 69%–169%. No conflict with slide +/- (iframes capture those).
