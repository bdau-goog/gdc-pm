#!/bin/bash
# =============================================================================
# GDC-PM — Train BigQuery ML Model: Gas Turbine Generator Classifier
# =============================================================================
# Trains a BOOSTED_TREE_CLASSIFIER on turbine_telemetry_raw to classify:
#   0 = normal
...
#
# Prerequisites:
#   - Turbine data seeded: python3 scripts/seed-turbine-data.py --project gdc-pm
#   - gcloud auth application-default login
#
# Usage:
#   bash scripts/train-turbine-model.sh
...
if [ -z "${PROJECT_ID:-}" ]; then
    ...
    echo "   Usage: bash scripts/train-turbine-model.sh --project YOUR_PROJECT_ID"
    exit 1
fi

DATASET_ID="grid_reliability_gold"
MODEL_ID="turbine_failure_classifier"
GCS_BUCKET="${PROJECT_ID}-models"
EXPORT_PATH="gs://${GCS_BUCKET}/turbine_classifier/$(date +%Y%m%d_%H%M%S)/"
LATEST_PATH="gs://${GCS_BUCKET}/turbine_classifier/latest/"

GREEN='\033[0;32m'; CYAN='\033[0;36m'; BOLD='\033[1m'; DIM='\033[2m'; RESET='\033[0m'

echo ""
echo -e "${BOLD}═══════════════════════════════════════════════════════════════════${RESET}"
echo -e "${BOLD}  GDC-PM — Turbine BQML Model Training & Export${RESET}"
echo -e "${BOLD}  Project: ${CYAN}${PROJECT_ID}${RESET}"
echo -e "${BOLD}═══════════════════════════════════════════════════════════════════${RESET}"
echo ""

# ── Step 1: Train ─────────────────────────────────────────────────────────────
echo -e "${BOLD}🚀 Step 1: Training turbine_failure_classifier (5–10 minutes)...${RESET}"
...
FROM
  \`${DATASET_ID}.turbine_telemetry_raw\`
...
FROM ML.EVALUATE(MODEL \`${DATASET_ID}.${MODEL_ID}\`,
  (SELECT psi, temp_f, vibration, is_failure FROM \`${DATASET_ID}.turbine_telemetry_raw\`)
)
...
# ── Step 3: Grant BQ service agent access to GCS ────────────────────────────
...
# ── Step 4: Export Model ──────────────────────────────────────────────────────
echo -e "${BOLD}📦 Step 3: Exporting turbine model to GCS...${RESET}"
bq query \
  --use_legacy_sql=false \
  --project_id="${PROJECT_ID}" \
...
echo "${LATEST_PATH}" > "${SCRIPT_DIR}/../gke/inference-api/.turbine-model-gcs-path"

echo ""
echo -e "${BOLD}═══════════════════════════════════════════════════════════════════${RESET}"
echo -e "${GREEN}${BOLD}  ✅ turbine_classifier trained and exported.${RESET}"
